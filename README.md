...
Optionally, there is also now a Self-Destruct function. While the Charge Beam is equipped and Power-Bombs are selected, hold the 'Movement Up' Button ('W' on the keyboard by default), and begin charging a shot (holding 'Movement Up' and 'Quick-Fire Power-Bomb' together will also work. Aiming up by holding the two aiming buttons will not work in place of the 'Movement Up' button.). After 2 seconds, Samus's health will start decreasing for as long as you hold this button combination, or until samus has only 10 hitpoints between her health and Reserve Tanks. I've added this to make the Crystal Flash ability more tactical, as it can only be activated when Samus has less than 50 health. Previously, if you were low on health but not quite low enough, you could stand around and wait for enemies to chip down your health, then use the Crystal Flash to heal yourself. This accomplishes the same goal, but faster. Of course, in doing this, you are taking a risk and and temporarily leaving yourself more vulnerable than you (probably) already are, so use wisely. If you don't like this function, it can be disabled.
